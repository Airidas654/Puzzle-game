...
public class PauseUi : MonoBehaviour
{
    ...
    [SerializeField] private LayerMask postProcessMask;

    private float defaultY;

    public static PauseUi instance;

    private void LerpPause(float val)
    {
    ...
    private void SetPauseInstantly(bool val)
    {
        transitioning = false;
        if (val)
        {
    ...
    {
        if (!transitioning && !GameManager.inst.dead && !GameManager.inst.CantDoAnything &&
            !GameManager.inst.levelEnterFreeze)
        {
            transitioning = true;
            DOTween.Kill(555);
            if (val)
            {
                ...
                blackScreen.gameObject.SetActive(true);
                list.gameObject.SetActive(true);
                DOTween.To(() => pauseVal, (x) =>
                {
                    pauseVal = x;
                    LerpPause(pauseVal);
                }, 1, 0.5f).SetId(555).OnComplete(() =>
                {
                    SetPauseInstantly(true);
                    transitioning = false;
                }).SetUpdate(true).SetUpdate(UpdateType.Normal, true);
            }
            else
            {
                DOTween.To(() => pauseVal, (x) =>
